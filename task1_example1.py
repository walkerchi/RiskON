...
color = "dodgerblue" # color for the curve
scatter_color = "orange" # color for the observations
scatter_marker = "x" # marker for the observations
...
def add_noise(x, std:float=0.1):
    return x * (1 + np.random.normal(0, std, len(x)))
...
observations = ax.scatter([], [], s=scatter_size, c=scatter_color, marker=scatter_marker, label="Observations")
...
def init():
    line.set_data([], [])
    observations.set_offsets(np.c_[[], []])
...
def update(frame):
    ...
    observations.set_offsets(np.c_[dates[:frame], new_y])
